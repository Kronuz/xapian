...
#ifndef XAPIAN_INCLUDED_NET_REMOTEPROTOCOL_H
#define XAPIAN_INCLUDED_NET_REMOTEPROTOCOL_H
...
struct MatchState {
    Xapian::Database *db;
    MultiMatch *match;
    std::vector<Xapian::Internal::opt_intrusive_ptr<Xapian::MatchSpy>> spies;
    Xapian::Weight *wt;
    MatchState() : db(NULL), match(NULL), wt(NULL) {}
    ~MatchState() {
    	reset();
    }
    void reset() {
...
class XAPIAN_VISIBILITY_DEFAULT RemoteProtocol::Internal : public Xapian::Internal::intrusive_base {
  ...
  protected:
    ...
    message_type get_message(double timeout, std::string & result,
				     message_type required_type_ = MSG_MAX) {
    	message_type type = static_cast<message_type>(remoteprotocol->get_message(timeout, result, required_type_));
	if (type < 0)
    ...
    void send_message(reply_type type, const std::string &message) {
    	...
    	remoteprotocol->send_message(type, message, end_time);
    }
    ...
    Xapian::Database * get_db() {
    	return remoteprotocol->get_db();
    }

    Xapian::WritableDatabase * get_wdb() {
    	return remoteprotocol->get_wdb();
    }

    void release_db(Xapian::Database *db) {
    	remoteprotocol->release_db(db);
    }

    void select_db(const std::vector<std::string> &dbpaths_, bool writable_, int flags) {
    	remoteprotocol->select_db(dbpaths_, writable_, flags);
    }

    void shutdown() {
    	remoteprotocol->shutdown();
    }
  ...
  public:
    ...
    Internal(RemoteProtocol *remoteprotocol_,
    ...
    void send_greeting();
    void send_exception(const std::string & err);
};

#endif // XAPIAN_INCLUDED_NET_REMOTEPROTOCOL_H

...
ProgClient::ProgClient(const string &progname, const string &args,
		       double timeout_, bool writable_, int flags_, const string & dir)
	: RemoteDatabase(run_program(progname, args
#ifndef __WIN32__
						   , pid
#endif
	),
			 timeout_, get_progcontext(progname, args), writable_,
			 flags_, dir)
{
    LOGCALL_CTOR(DB, "ProgClient", progname | args | timeout_ | writable_ | flags_ | dir);
}
